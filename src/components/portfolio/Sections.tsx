...
function SectionTitle({ main, script }: { main: string; script: string }) {
  return (
    <div className="reveal mb-10 sm:mb-12">
      <h2 className="font-display text-yellow text-5xl tracking-wide uppercase drop-shadow-lg sm:text-7xl md:text-8xl">{main}</h2>
      {script ? <p className="font-script -mt-3 ml-1 text-2xl text-white drop-shadow sm:-mt-6 sm:text-4xl">{script}</p> : null}
    </div>
  );
}
    ...
    <section id="home" className="relative overflow-hidden scroll-mt-24">
      <div className="mx-auto flex min-h-[78vh] max-w-7xl flex-col items-center justify-center gap-8 px-5 py-12 sm:min-h-[85vh] sm:gap-10 sm:px-6 sm:py-16 lg:flex-row lg:gap-16">
        <div className="reveal flex-1 text-center lg:text-left">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-white/80 sm:mt-5 sm:text-lg lg:mx-0">I help ambitious businesses build stronger brands, create consistent digital content, and turn creative ideas into effective marketing visuals.</p>
          <div className="mt-7 flex flex-wrap justify-center gap-3 sm:mt-10 sm:gap-4 lg:justify-start">
            ...
            <a href="#contact" className="font-display hover:text-blue-deep rounded-md border-2 border-white px-6 py-3 text-base tracking-wide text-white transition hover:bg-white sm:px-8 sm:text-lg">Start a Project</a>
          </div>
        </div>
  ...
  return (
    <section id="about" className="mx-auto max-w-7xl scroll-mt-24 px-5 py-12 sm:px-6 sm:py-16">
      ...
      <div dir="rtl" className="font-arabic mt-12 grid grid-cols-2 gap-x-5 gap-y-8 sm:mt-16 sm:gap-x-8 sm:gap-y-12 lg:grid-cols-4">
        {stats.map((stat) => <div key={stat.label} className="reveal text-center"><p className="font-display text-yellow text-5xl drop-shadow-lg sm:text-7xl">{stat.value}</p><p className="mt-2 text-base font-semibold text-white sm:text-xl">{stat.label}</p></div>)}
      </div>
    </section>
  );
}
...
export function Skills() {
  return <section id="skills" className="mx-auto max-w-7xl scroll-mt-24 px-5 py-12 sm:px-6 sm:py-16"><SectionTitle main="My" script="Skills & Expertise" /><h3 className="font-display text-yellow reveal mb-5 text-2xl tracking-widest uppercase sm:mb-6 sm:text-3xl">Software</h3><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4">{software.map((item) => <div key={item.name} className="reveal flex items-center gap-3 rounded-xl bg-white/10 p-4 backdrop-blur-sm"><Ring value={item.value} /><span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white/10"><img src={item.icon} alt={`${item.name} icon`} loading="lazy" width={44} height={44} className="h-9 w-9 object-contain drop-shadow-md" /></span><p className="text-base leading-tight font-medium sm:text-lg">{item.name}</p></div>)}</div><div className="mt-12 grid gap-10 sm:mt-16 sm:gap-12 lg:grid-cols-2"><div className="reveal"><h3 className="font-display text-yellow mb-5 text-2xl tracking-widest uppercase sm:mb-6 sm:text-3xl">Skills</h3><ul className="grid gap-3 text-base sm:grid-cols-2 sm:text-lg">{skills.map((skill) => <li key={skill} className="flex items-center gap-3"><span className="bg-yellow h-2.5 w-2.5 shrink-0 rounded-full" />{skill}</li>)}</ul></div><div className="reveal"><h3 className="font-display text-yellow mb-5 text-2xl tracking-widest uppercase sm:mb-6 sm:text-3xl">Language</h3><ul className="space-y-3 text-base sm:text-lg">{languages.map((lang) => <li key={lang.name} className="flex items-center gap-3"><span className="bg-yellow h-2.5 w-2.5 shrink-0 rounded-full" />{lang.name} : {lang.level}</li>)}</ul></div></div></section>;
}
...
export function Contact() {
  ...
  return <section id="contact" className="mx-auto max-w-7xl scroll-mt-24 px-5 py-16 sm:px-6 sm:py-20"><div className="grid items-center gap-10 sm:gap-14 lg:grid-cols-2"><div className="reveal"><p className="text-yellow font-script text-3xl sm:text-5xl">Ready to Work Together?</p><h2 className="font-display mt-2 text-5xl leading-tight tracking-wide text-white uppercase drop-shadow-lg sm:text-7xl">Let&apos;s Create<br />Something Great</h2><p className="text-yellow mt-6 max-w-md text-base leading-relaxed font-semibold tracking-wide uppercase sm:mt-8 sm:text-lg">Branding, social media design, and digital marketing support for businesses that want to stand out.</p><p className="mt-3 text-base tracking-wide uppercase sm:mt-4 sm:text-lg">Tell me about your next project and let&apos;s build it together.</p></div><ul className="space-y-3 sm:space-y-5">{items.map(({ icon: Icon, label, href, external }) => <li key={label} className="reveal"><a href={href} {...(external ? { target: "_blank", rel: "noreferrer" } : {})} className="flex items-center gap-3 rounded-xl bg-white/10 p-4 backdrop-blur-sm transition hover:bg-white/20 sm:gap-4 sm:p-5"><span className="bg-yellow flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-black sm:h-12 sm:w-12"><Icon size={21} /></span><span className="break-all text-base sm:text-lg">{label}</span></a></li>)}</ul></div></section>;
}
